...
#For apt_buildings dataset
glimpse(apt_buildings)
```
```{r}

#For vancouver_trees dataset
glimpse(vancouver_trees)
```
```{r}

#For building_permits dataset
...
<!-------------------------- Start your work below ---------------------------->
I am going to choose the cancer_sample datatset. Since I am a student in the Genome Science Department, this dataset fits my research interest the best.
...
<!-------------------------- Start your work below ---------------------------->
I think my research question will be: **"What are the characteristics of the size of the tumor in patients with malignant cancer?"**
<!----------------------------------------------------------------------------->
...
```{r}
#1
malignant_patient <- cancer_sample %>%
  filter(diagnosis == "M")

ggplot(malignant_patient, aes(x = radius_mean)) +
  geom_histogram(fill = "blue", color = "black", alpha = 0.5) +
  labs(title = "The Overall Radius Mean in Patients with Malignant Tumor", x = "Radius Mean", y = "Frequency") +
  theme_minimal()

#In this code, I am exploring the mean tumor size in patients with a malignancy diagnosis. This will allow researchers to get a better understanding of what size of the tumor they should be aware of when analyzing malignant tumor
```
```{r}
#4

ggplot(malignant_patient, aes(x = radius_mean, y = area_mean)) +
  geom_point(color = "blue", alpha = 0.6) +
  labs(title = "Relationship Between Radius Mean and Area Mean in Malignant Tumors", 
       x = "Radius Mean", y = "Area Mean") +
  theme_minimal()

#In this code, I am determining the relationship between the radius mean and area mean in malignant tumors. Tumor size and shape are crucial in diagnosing cancer and determining treatment options. I used a scatter plot to assess whether there is a correlation between the radius and area of malignant tumors, such as whether larger tumors tend to have larger radius values.
```

```{r}
#5

# Filter observations with radius_mean greater than 20
big_malignant <- malignant_patient %>% 
  filter(radius_mean > 20)

# Display the filtered data
head(big_malignant)

#In this code, I am filtering out the tumors with a radius mean greater than 20. By doing so, I can concentrate my analysis on a group of tumors that might be more aggressive, which can be critical for developing treatment plans.
```

```{r}

#6
# Create a boxplot for area_mean by diagnosis
...
#In this code, I am creating a boxplot for area mean by diagnosis. This will provide a clear visual representation of the distribution of tumor sizes across malignant vs benign patients.
```
...
<!--- *****START HERE***** --->
For my next milestone, I would like to compare the difference between benign and malignant tumor's characteristics. Specifically, here are my four questions:

1/ How does the area and smoothness of tumors differ between malignant and benign diagnoses?
2/ Do certain characteristics (like area, smoothness, and radius) show consistent trends in malignant tumors compared to benign ones?
3/ How many patients are benign despite showing characteristics of malignant tumor?
4/ What is the acceptable parameters for a patient to be diagnosed as malignant?
<!----------------------------->
